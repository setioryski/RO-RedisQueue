...
const uploadQueue = require('./uploadProcessor'); // Import the queue from the
...
app.post('/upload', isAuthenticated, checkRole(['admin', 'petugas']), upload.single('foto'), async (req, res) => {
    ...
    const jobData = {
        ...
        resizedImagePath: `uploads/resized-${req.file.filename}`,
        id_kondisi,
        catatan,
        id_user,
        id_tipe_lantai,
    ...
    try {
        ...
        // Respond immediately
        res.status(200).json({ success: true, message: 'Upload received, processing will continue in the background.' });
